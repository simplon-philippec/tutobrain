...
$apiLinkBrico = 'https://api.dailymotion.com/videos?fields=description,embed_url,embed_html,thumbnail_url,title,comments_total,views_total,&channel=lifestyle&detected_language=fr&tags=bricolage+tuto+tutoriel&limit=10';
$request = file_get_contents($apiLinkBrico);
$requestDecode = json_decode($request);

echo "<h1>Tuto Brico :</h1>";
...
foreach ($requestDecode->list as $key => $value) {
  ...
  echo "<p class='bottom-text-content'>Nombre de Commentaires : " .$requestDecode->list[$key]->comments_total. "</p><br/></div>";
}
